...
export const Card: ComponentConfig<CardProps> = {
    ...
    render: ({ heading, snippet, mediaType, iconName, iconBgColor, imageSrc, buttonDisabled, buttonText, buttonLink, buttonIcon }) => {
        const buttonClasses = `
            inline-flex items-center justify-center px-6 py-3 border border-transparent text-base font-medium rounded-md
            transition-colors duration-200
            text-blue-700 bg-blue-100 hover:bg-blue-200 // Always use enabled styles if rendered
        `;
        ...
        return (
            <div className={`bg-white p-8 rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 ease-in-out transform hover:-translate-y-1
                flex flex-col h-full`}>

                {mediaType === 'icon' && IconComponent && (
                    <div className={`flex items-center justify-center w-16 h-16 text-white bg-black ${iconBgColor} rounded-full mb-6 mx-auto text-3xl`}>
                ...
                <p className="mb-4" dangerouslySetInnerHTML={{ __html: snippet }}/>
